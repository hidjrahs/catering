<script>
    ...
    $(function() {
        ...
        table = $('#main-table').DataTable({
            ...
            orderable: false,
            ajax: {
                url: "{{ route('web.suppliers.all-paginate') }}?device=web",
                method: 'GET',
                data: function(d) {
                    d.search = $('#search').val();
            ...
            columns: [{
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        return '<span class="text-dark fw-bold text-hover-primary d-block mb-1 fs-6">'+data+'</span>'+
                            '<span class="text-muted fw-semibold text-muted d-block fs-7">'+(row.desc?'Ket: '+row.desc:'-')+'</span>';
                    }
        ...
        const clearModalUpdate=function(){
            ...
            formUpdate.find('[name="penanggung_jawab"]').val('');
            ...
            formUpdate.find('[name="penanggung_jawab"]').val(parameter.penanggung_jawab);
        ...
        $(document).on('click', '[data-ref]', function(event) {
            ...
            blockUIBody.block();
            clearModalUpdate();
            asyncData("{{ url('web/suppliers') }}/"+refId,{},"GET").then((response) => {
                if (!response.status) {
                    blockUIBody.release();
                    return Swal.fire({icon: 'error',text: response.message});
                }
                const resultlist = response.data;
                filledModalUpdate(resultlist, "{{ url('web/suppliers') }}/"+refId);
                blockUIBody.release();
        ...
        $(document).on('click', '#deleted-check:not(.disabled)', function() {
            ...
            Swal.fire({
                ...
                if (willsend.isConfirmed) {
                    blockUIBody.block();
                    asyncData("{{route('web.suppliers.destroy')}}",values,"POST").then((response) => {
