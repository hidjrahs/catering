<script>
    ...
    $(function() {
        ...
        table = $('#main-table').DataTable({
            ...
            orderable: false,
            ajax: {
                url: "{{ route('web.category_menus.all-paginate') }}?device=web",
                method: 'GET',
                data: function(d) {
                    d.search = $('#search').val();
            ...
            'createdRow': function( row, data, dataIndex ) {
                $(row).addClass('cursor-pointer')
                    .attr('title','Edit Supplier : '+data.name+' ?')
            ...
            columns: [{
                    ...
                    render: function(data, type, row, meta) {
                        if(row.pathfile){
                            symb='<img src="'+row.pathfile+'" class="h-75 align-self-end" alt="">';
                        }else if(row.icon){
                            symb='<i class="fa-solid '+row.icon+' fs-2"></i>';
                        }else{
                            symb=data.trim()
                                .split(/\s+/)
        ...
        const clearModalUpdate=function(){
            ...
            formUpdate.find('[data-place="filename"]').html('No file selected.');
            formUpdate.find('.preview-image>span').html('');
            formUpdate.find('.preview-image').addClass('hidden');
            formUpdate.find('[name="has_image"]').prop("checked", true);
            formUpdate.find('[name="is_quantity"]').prop('checked',false)
            ...
            formUpdate.find('[data-place="filename"]').html(parameter.filename??'No file selected.');
            if(parameter.pathfile){
                imageU="<img src='"+parameter.pathfile+"'></img>";
                formUpdate.find('.preview-image>span').html(imageU);
                formUpdate.find('.preview-image').removeClass('hidden');
                formUpdate.find('[name="has_image"]').prop("checked", true);
            }else{
                formUpdate.find('[name="has_image"]').prop("checked", false);
            }
            formUpdate.find('[name="is_quantity"]').prop('checked',parameter.is_quantity??false)
        };
        $(document).on('click', '[data-ref]', function(event) {
            ...
            blockUIBody.block();
            clearModalUpdate();
            asyncData("{{ url('web/category_menus') }}/"+refId,{},"GET").then((response) => {
                if (!response.status) {
                    blockUIBody.release();
                    return Swal.fire({icon: 'error',text: response.message});
                }
                const resultlist = response.data;
                filledModalUpdate(resultlist, "{{ url('web/category_menus') }}/"+refId);
                blockUIBody.release();
        ...
        $(document).on('click', '#deleted-check:not(.disabled)', function() {
            ...
            Swal.fire({
                ...
                if (willsend.isConfirmed) {
                    blockUIBody.block();
                    asyncData("{{route('web.category_menus.destroy')}}",values,"POST").then((response) => {
