...
class  CostStructureRepository
{
    ...
    public static function search($request){
        ...
        $where="(lower(cost_structure.name) like '%".$lowerSearch."%')";
        $q = $request->get('q');
        $result = CostStructure::query()
            ->when($q, fn($query) => $query->whereRaw($where))
            ->active()
            ->select(['cost_structure.id','cost_structure.name'])
            ->with(['detail:id,cost_structure_id,name,fixed,kategori,prosentase'])
            ->get();
        $regenerate=collect($result)->map(function ($item) {
            return [
                'id'     => $item['id'],
                'name'   => $item['name'],
