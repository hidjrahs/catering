...
class EmployesApiController extends Controller
{
    use Validators;
    protected $EmployesRepository;
    public function __construct(EmployesRepository $EmployesRepository)
    {
        $this->EmployesRepository = $EmployesRepository;
    }
    public function index(Request $request){
        try {
            $result=$this->EmployesRepository->getallData($request);
            if(!$result){
    ...
    public function store(Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->EmployesRepository->store($request);
            if(!$result){
    ...
    public function update($refId,Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->EmployesRepository->update($refId,$request);
            if(!$result){
    ...
    public function destroy(Request $request){
        DB::beginTransaction();
        try {
            $result=$this->EmployesRepository->delete($request);
            if(!$result){
    ...
    public function show($refId,Request $request){
        try {
            $result=$this->EmployesRepository->detail($refId,$request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
