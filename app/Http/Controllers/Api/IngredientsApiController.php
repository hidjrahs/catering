...
class IngredientsApiController extends Controller
{
    ...
    public function index(Request $request){
        try {
            $result=$this->IngredientsRepository->getallData($request);
            if(!$result){
    ...
    public function store(Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->IngredientsRepository->store($request);
            if(!$result){
    ...
    public function search(Request $request){
        try {
            $result=$this->IngredientsRepository->search($request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
    ...
    public function update($refId,Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->IngredientsRepository->update($refId,$request);
            if(!$result){
    ...
    public function destroy(Request $request){
        DB::beginTransaction();
        try {
            $result=$this->IngredientsRepository->delete($request);
            if(!$result){
    ...
    public function show($refId,Request $request){
        try {
            $result=$this->IngredientsRepository->detail($refId,$request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
