...
class CustomersApiController extends Controller
{
    ...
    public function index(Request $request){
        try {
            $result=$this->CustomersRepository->getallData($request);
            if(!$result){
    ...
    public function store(Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->CustomersRepository->store($request);
            if(!$result){
    ...
    public function update($refId,Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->CustomersRepository->update($refId,$request);
            if(!$result){
    ...
    public function destroy(Request $request){
        DB::beginTransaction();
        try {
            $result=$this->CustomersRepository->delete($request);
            if(!$result){
    ...
    public function show($refId,Request $request){
        try {
            $result=$this->CustomersRepository->detail($refId,$request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
    ...
    public function search(Request $request){
        try {
            $result=$this->CustomersRepository->search($request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
