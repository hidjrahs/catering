...
{
    use Validators;
    protected $PurchasingRepository;
    public function __construct(PurchasingRepository $PurchasingRepository)
    {
        $this->PurchasingRepository = $PurchasingRepository;
    }
    public function index(Request $request){
        try {
            $result=$this->PurchasingRepository->getallData($request);
            if(!$result){
    ...
    public function batch(Request $request){
        try {
            $result=$this->PurchasingRepository->getallDataBatch($request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
    ...
    public function store(Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->PurchasingRepository->store($request);
            if(!$result){
    ...
    public function batch_report(Request $request){
        try {
            $result=$this->PurchasingRepository->exportBatch($request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            return  $this->successResponse('Data di temukan.',['data'=>$result]);
    ...
    public function update($refId,Request $request){
        ...
        DB::beginTransaction();
        try {
            $result=$this->PurchasingRepository->update($refId,$request);
            if(!$result){
                throw new Exception('Data gagal di simpan.', 1);
            }
            DB::commit();
    ...
    public function export($refId,Request $request){
        try {
            $result=$this->PurchasingRepository->export($refId,$request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            return  $this->successResponse('Data di temukan.',['data'=>$result]);
    ...
    public function show($refId,Request $request){
        try {
            $result=$this->PurchasingRepository->detail($refId,$request);
            if(!$result){
                throw new Exception('Data tidak ditemukan.', 1);
            }
            if(in_array(request('device'),['web','stealth','mobile'])){
